...
'REPLACE_COLOR(ocol as long,ncol as long,xwidth as integer,ywidth as integer,ibuff as any ptr)
'DELIM2(stringz as string,delimiter as string,arrayz() as string)
'SAVE_BUFFER(dbuff as any ptr,filen as string)
dim imagez(1 to 2) as sprite
...
dim ni as any ptr
print
print "enclose filename in batch with * as delimiters"
print
input "Input File Name containing Batch>",filen
dim as integer oldr,oldg,oldb,newr,newg,newb 
input "Input old Red>",oldr
input "Input old Green>",oldg
input "Input old Blue>",oldb
input "Input new Red>",newr
input "Input new Green>",newg
input "Input new Blue>",newb
input "Input Minimum Y axis>",ry
SET_SCREEN 640,480,32, "Batch Color & Y axis Changer v1.0 by Matt B." 
dim f1 as long
f1=freefile
open filen for input as #f1
...
line input #f1,z
delim2 z,"*",a()
LOAD_SPRITE imagez(1),a(2)
'print a(2)
cls
put (0,0),imagez(1).spritebuf,PSET

if imagez(1).ywidth<ry then
cls
paint (0,0),RGB(newr,newg,newb)
put (0,(ry-imagez(1).ywidth)),imagez(1).spritebuf,PSET
''SAVE_BUFFER imagez(1).spritebuf,"test1.bmp"
'line (0,0)-(imagez(1).xwidth-1,ry+10),rgb(255,255,255),b
'BSAVE "test1.bmp",0
'SLEEP
ImageDestroy imagez(1).spritebuf
imagez(1).ywidth=ry
imagez(1).spritebuf=ImageCreate(imagez(1).xwidth+1,imagez(1).ywidth,RGB(newr,newg,newb))  
get (0,0)-(imagez(1).xwidth-1,imagez(1).ywidth-1),imagez(1).spritebuf
end if    
REPLACE_COLOR RGB(oldr,oldg,oldb),RGB(newr,newg,newb),imagez(1).xwidth,imagez(1).ywidth,imagez(1).spritebuf
'cls
put (0,0),imagez(1).spritebuf,PSET
'SAVE_BUFFER(dbuff as any ptr,filen as string)
SAVE_BUFFER imagez(1).spritebuf,a(2)
'sleep
ImageDestroy imagez(1).spritebuf
loop until eof(f1)
close f1
